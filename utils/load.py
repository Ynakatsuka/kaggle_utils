...
def check_columns(df, columns):
    ...
    return df[non_duplicated], columns
    
    
def load_features(file_names, data_dir='../data/working/features/'):
    features = []
    columns = []
    for file_name in file_names:
        ...
        if extension == 'ftr':
            df, columns = check_columns(pd.read_feather(load_paths[0]), columns)
            features.append(df)
        elif extension == 'pkl':
            df, columns = check_columns(pd.read_pickle(load_paths[0]), columns)
            features.append(df)
        elif extension == 'npy':
            feature = np.load(load_paths[0])
            npy_columns = [file_name + str(i) for i in range(features.shape[1])]
            df = pd.DataFrame(feature, columns=npy_columns)
            df, columns = check_columns(df, columns)
            features.append(df)
        else:
